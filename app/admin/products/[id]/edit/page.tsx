          ...
          <div className="flex flex-wrap gap-3">
            {(['DRAFT', 'ACTIVE', 'ARCHIVED'] as const).map(s => {
              const styles: Record<string, string> = {
                DRAFT: status === 'DRAFT' ? 'border-yellow-400 bg-yellow-50 text-yellow-700' : '',
                ACTIVE:
                  status === 'ACTIVE' ? 'border-emerald-400 bg-emerald-50 text-emerald-700' : '',
                ARCHIVED: status === 'ARCHIVED' ? 'border-gray-400 bg-gray-100 text-gray-700' : '',
              };
              return (
                <button
                  key={s}
                  ...
                  className={`py-2 px-4 rounded-lg border text-sm font-medium transition-all ${
                    styles[s] || 'border-gray-200 bg-white text-gray-500 hover:border-gray-300'
                  }`}
                  onClick={() => {
                    setStatus(s);
                    setIsDirty(true);
